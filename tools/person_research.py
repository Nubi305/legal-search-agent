"""
Person research tool using the legal search agent.
"""
...
def main():
    """Run the person research tool."""
    # Load environment variables
    load_dotenv()
    
    # Parse arguments
    parser = argparse.ArgumentParser(description="Person Research Tool")
    parser.add_argument("--name", required=True, help="Person's name")
    parser.add_argument("--state", help="State abbreviation (e.g., FL, NY, CA)")
    parser.add_argument("--profession", help="Professional context (e.g., lawyer, doctor, business)")
    parser.add_argument("--refresh", action="store_true", help="Refresh data (recrawl)")
    parser.add_argument("--output", default="person_research", help="Output directory")
    ...
    if args.refresh or not os.listdir(data_dir):
        print(f"\n--- Collecting data for {args.name} ---")
        
        # Load configuration
        config = Config.from_file("configs/person_research.json")
        crawler = LegalCrawler(config)
        
        # Modify crawler behavior to target specific person
        # This is a simple approach - you might want to implement more sophisticated targeting
        person_query = args.name
        if args.state:
            person_query += f" {args.state}"
        if args.profession:
            person_query += f" {args.profession}"
        
        print(f"Searching for: {person_query}")
        
        # Here we'd ideally modify the crawler to search specifically for the person
        # This would require extending the crawler class to support search queries
        # For now, we'll just crawl the standard sources
        
        print("Crawling sources for person information...")
    ...
    # Step 3: Index
    print("\n--- Indexing person data ---")
    indexer = DocumentIndexer()
    indexer.index_directory(processed_dir, index_dir)
    print(f"Indexing complete. Index saved to {index_dir}")
    
    # Step 4: Search
    print(f"\n--- Researching {args.name} ---")
    
    # Basic person queries
    queries = [
        f"{args.name} professional licenses" + (f" in {args.state}" if args.state else ""),
        f"{args.name} legal records",
        f"{args.name} court cases",
        f"{args.name} " + (args.profession if args.profession else "professional background")
    ]
    
    # Use LangChain for more sophisticated research
    try:
        if os.getenv("OPENAI_API_KEY"):
            print("\n=== Person Background Report ===\n")
            
            legal_langchain = LegalLangChain(index_dir)
            
            # Comprehensive query
            profession_context = f"with focus on their {args.profession} background" if args.profession else ""
            state_context = f"in {args.state}" if args.state else ""
            
            comprehensive_query = f"""
            Provide a comprehensive background report on {args.name} {profession_context} {state_context}.
            Include the following information if available:
            1. Professional licenses and their status
            2. Any legal proceedings they've been involved in
            3. Professional positions and affiliations
            4. Any regulatory actions or disciplinary history
            5. Recent court appearances or legal matters
            """
            
            print("Generating person background report...\n")
            response = legal_langchain.query(comprehensive_query)
            
            print("Person Background Report:")
            print("-" * 50)
    ...
    print(f"python main.py search --query \"your query about {args.name}\" --index {index_dir}")
